/* Fusion Flavours - single runtime bootstrap
   Owns app routing and Harnell presentation. Delivery owns itself in delivery-management.js.
*/
(() => {
  'use strict';
  ...
  function hideTopViews(){
    ['welcomeHub','customer','retailCustomer','owner','cateringCustomer','harnellCustomer','legalCustomer'].forEach(id => {
      const el = byId(id);
  ...
  function showHarnell(updateUrl = true){
    hideTopViews();
    closeHeaderMenu();
    const page = byId('harnellCustomer');
  ...
  function harnellDishCard(row){
    ...
    const displayName = typeof harnellDisplayName === 'function'
      ? harnellDisplayName(row)
      : (row.resident_name || row?.items?.name || 'Dish');
    const displayDescription = typeof harnellDisplayDescription === 'function'
      ? harnellDisplayDescription(row)
    ...
    return '<article class="harnellDish">' +
      ...
      '<h3>' + safe(displayName) + '</h3><p class="muted">' + safe(displayDescription) + '</p>' +
      '<div class="row"><span class="harnellPrice">' + fmtMoney(Number(row.resident_price_pence || 0) / 100) +
      '</span><button ' + (!canOrder ? 'disabled' : '') + ' onclick="addHarnellItem(' + Number(row.item_id) + ')">' +
      (soldOut ? 'Sold out' : 'Add') + '</button></div></article>';
  }
  ...
  function installHarnellPresentation(){
    ...
    window.renderHarnellCustomer = function(){
      ...
      const start = String(s.harnell_delivery_start || '18:00').slice(0,5);
      const end = String(s.harnell_delivery_end || '20:00').slice(0,5);
      if (byId('harnellStatus')) {
        byId('harnellStatus').innerHTML =
          (harnellOpen() ? '<b>🟢 Resident ordering open</b> · Order by ' : '<b>🔴 Resident ordering closed</b> · Cutoff ') +
          String(s.harnell_cutoff_time || '15:00').slice(0,5) + '<br><b>🚪 Delivery window:</b> ' + start + '–' + end;
      }

      const groups = [['mains','Mains'],['sides','Sides'],['drinks','Drinks'],['desserts','Desserts']];
      const menu = byId('harnellMenu');
      if (!menu) return;
      menu.className = '';
      menu.innerHTML = groups.map(([key,label]) => {
        ...
        return '<section class="harnellSection" data-harnell-section="' + key + '">' +
          '<h2 class="harnellSectionTitle">' + label + '</h2><div class="harnellMenuGrid">' +
  ...
  function injectRuntimeStyles(){
    ...
    style.textContent = '.harnellSection{margin:24px 0 12px}.harnellSectionTitle{margin:0 0 12px;padding:9px 2px 8px;border-bottom:4px solid var(--o);font-size:25px;font-weight:950}.harnellMenuGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-bottom:4px}.harnellEmpty{grid-column:1/-1;background:#fff8ef;border:1px dashed #d8c2ae;border-radius:14px;padding:14px;color:var(--muted);font-size:13px}@media(max-width:760px){.harnellMenuGrid{grid-template-columns:1fr}}';
