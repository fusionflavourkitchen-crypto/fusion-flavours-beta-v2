...
(() => {
  ...
  async function renderHarnell() {
    ...
    page.innerHTML = `<h2>Prep</h2><div class="prepModeTabs"><button data-prep-mode="takeaway">Takeaway</button><button data-prep-mode="catering">Catering</button><button class="active" data-prep-mode="harnell">Harnell</button></div><div class="notice"><b>Harnell prep</b> · Today's open resident orders only.</div><div class="panel"><h2>Portions to prepare</h2>${Object.entries(totals).map(([name, qty]) => `<div class="prepRow"><b>${safe(name)}</b><div class="prepQty">× ${qty}</div></div>`).join('') || '<p class="muted">No Harnell prep required.</p>'}</div><div class="panel" style="margin-top:12px"><h2>Recipe ingredient requirements</h2>${Object.values(requirements).map(item => `<div class="prepRow"><b>${safe(item.name)}</b><div class="prepQty">${Number(Number(item.qty || 0).toFixed(3))} ${safe(item.unit)}</div></div>`).join('') || '<p class="muted">No linked recipe requirements.</p>'}</div>`;
