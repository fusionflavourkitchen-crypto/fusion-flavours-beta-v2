...
(() => {
  ...
  function updateCustomerDepositNotice() {
    ...
    if (pack && guests) html += ` · ${guests} guests at ${moneyLocal(Number(pack.price_per_head || 0))} = ${moneyLocal(total)} total · <b>${moneyLocal(deposit)} deposit</b>`;
  ...
  async function copyBalanceLink(id) {
    try {
      ...
      } catch (_) { window.prompt('Copy this balance payment link:', result.url); }
  ...
  function enhanceBookingCards() {
    document.querySelectorAll('.cateringBooking').forEach(card => {
      ...
      </div><div class="split" style="margin-top:9px">
      ...
      </div><p class="muted" style="margin:8px 0 0">Policy: 50% secures the booking. Final balance is due 3 working days before the event.</p>`;
      if (!box) {
  ...
  const api = { apply, beforeRender, updateCustomerDepositNotice, setDeposit50, copyBalanceLink };
  window.FusionCateringPolicy = api;
  window.FusionOwnerRouter?.register?.('catering', { beforeRender, afterRender: apply });
